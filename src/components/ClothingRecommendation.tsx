...
const ClothingRecommendation = ({ recommendation, weatherData }) => {
  ...
  const renderSection = (section, index) => {
    ...
    const isWeatherSummary = title.toLowerCase().includes('weather summary');
    const isClothingRecommendation = title.toLowerCase().includes('clothing recommendation');
    ...
    return (
      <div key={index} className="mb-4">
        <h4 className="text-lg font-semibold mb-2">{title.replace(/^[#*]+\s*/, '').replace(/[:*]+$/, '')}</h4>
        <ul className="list-disc list-inside space-y-2">
          {content.map((item, itemIndex) => (
            <li key={itemIndex} className="text-gray-700">
              {item.trim().replace(/^[*•-]\s*/, '').replace(/[:*]+$/, '')}
            </li>
          ))}
        </ul>
      </div>
